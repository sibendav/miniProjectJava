...
public class SphereTest {
    ...
    public void testFindIntersections() {
        ...
        Intersectable.GeoPoint p1 =new Intersectable.GeoPoint(sphere, new Point3D(0.0651530771650466, 0.355051025721682, 0));
        Intersectable.GeoPoint p2 =new Intersectable.GeoPoint(sphere, new Point3D(1.53484692283495, 0.844948974278318, 0));
        List<Intersectable.GeoPoint> result = sphere.findIntsersections(new Ray(
                new Vector(3, 1, 0),new Point3D(-1, 0, 0)));
        assertEquals("Wrong number of points", 2, result.size());
        if (result.get(0).getPoint().getCoord1().get() > result.get(1).getPoint().getCoord1().get())
        ...
        Intersectable.GeoPoint p3=new Intersectable.GeoPoint(sphere,new Point3D(2,0,0));
        ...
        p1 = new Intersectable.GeoPoint(sphere,new Point3D(0,0,0));
        p2 = new Intersectable.GeoPoint(sphere,new Point3D(2, 0, 0));
        result = sphere.findIntsersections(new Ray(new Vector(1,0,0),new Point3D(-1, 0, 0)));
        assertEquals("Wrong number of points", 2, result.size());
        if (result.get(0).getPoint().getCoord1().get() > result.get(1).getPoint().getCoord1().get())
            result = List.of(result.get(1), result.get(0));
        ...
        p3=new Intersectable.GeoPoint(sphere,new Point3D(2,0,0));
        result=sphere.findIntsersections(new Ray( new Vector(1, 0, 0),new Point3D(0,0, 0)));
        assertEquals("Wrong number of points", 1, result.size());
        assertEquals("Ray's line inside the sphere", result.get(0), p3);
